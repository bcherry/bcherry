/*jslint white: true, onevar: true, browser: true, devel: true, undef: true, nomen: false, eqeqeq: true, plusplus: true, bitwise: true, regexp: false, strict: false, newcap: false, immed: true */
...
(function (global) {
	...
	function scriptTagExists(uri) { // checks if a script uri has ever been loaded into this page's DOM
		...
		while ((script = docScripts[idx])) {
			idx += 1;
			if (typeof script.src === sSTRING && uri === canonicalScriptURI(script.src) && script.type !== sSCRIPTCACHE) {
				return bTRUE;
			}
		}
		return bFALSE;
	...
	function engine(queueExec, opts) {
		...
		function handleScriptLoad(elem, scriptentry, skipReadyCheck) {
			...
			if (!skipReadyCheck && !(isScriptLoaded(elem, scriptentry))) {
				return;
			}
			...
			for (var key in scripts) {
				if (scripts[sHASOWNPROPERTY](key) && !(scripts[key][sDONE])) {
					return;
				}
			}
		...
		function handleScriptPreload(elem, scriptentry) {
			if (!isScriptLoaded(elem, scriptentry)) {
				return;
			}
		...
		function loadScriptCache(scriptentry, src, type, charset) {
			...
			if (first_pass && scriptentry[sPRELOADDONE] === uUNDEFINED) { // need to preload into cache
			...
			} else if (!first_pass && scriptentry[sPRELOADDONE] !== uUNDEFINED && !scriptentry[sPRELOADDONE]) { // preload still in progress, make sure trigger is set for execution later
		...
		function loadScriptXHR(scriptentry, src, type, charset) {
			...
			if (first_pass && scriptentry[sPRELOADDONE] === uUNDEFINED) { // need to preload
				scriptentry[sPRELOADDONE] = bFALSE;
				...
				xhr.open("GET", src);
				xhr.send("");
			} else if (!first_pass && scriptentry[sPRELOADDONE] !== uUNDEFINED && !scriptentry[sPRELOADDONE]) { // preload XHR still in progress, make sure trigger is set for execution later
				scriptentry[sLOADTRIGGER] = function () {
		...
		function loadScript(o) {
			...
			if (!allowDup && 
				(
					(all_scripts[src_uri] !== uUNDEFINED) || (first_pass && scripts[src_uri]) || scriptTagExists(src_uri)
				)
			) {
				if (scripts[src_uri] !== uUNDEFINED && scripts[src_uri][sPRELOADDONE] && !scripts[src_uri][sDONE] && same_domain) {
			...
			if (scripts[src_uri] === uUNDEFINED) {
				scripts[src_uri] = {};
			}
			
			scriptentry = scripts[src_uri];
			
			if (scriptentry[sWHICH] === uUNDEFINED) {
			...
			if (!_use_script_order && _use_xhr_preload && same_domain) {
				loadScriptXHR(scriptentry, src_uri, type, charset);
			} else if (!_use_script_order && _use_cache_preload) {
				loadScriptCache(scriptentry, src_uri, type, charset);
			} else {
		...
		function queueAndExecute(execBody) { // helper for publicAPI functions below
			...
			if (!queueExec || _use_preload) {
				execBody(); // if engine is either not queueing, or is queuing in preload mode, go ahead and execute
			}
		}
		...
		function serializeArgs(args) {
			...
			for (idx = 0; idx < args.length; idx += 1) {
				if (fOBJTOSTRING.call(args[idx]) === sTYPEARRAY) {
					sargs = sargs.concat(serializeArgs(args[idx]));
				} else {
					sargs[sargs.length] = args[idx];
				}
			}
			return sargs;
		}
				...
				if (_auto_wait) {
					for (idx = 0; idx < args.length; idx += 1) {
						if (idx === 0) {
							...
							use_engine = use_engine.script(args[idx]);
						}
	...
	function processOpts(opts) {
		...
		for (k in boolOpts) {
			if (boolOpts[sHASOWNPROPERTY](k)) {
				allOpts[k] = boolOpts[k];
			}
		}
		newOpts.order = !!global_defs.order;
		for (k in allOpts) {
			if (allOpts[sHASOWNPROPERTY](k) && global_defs[allOpts[k]] !== uUNDEFINED) {
				newOpts[allOpts[k]] = (opts[k] !== uUNDEFINED) ? opts[k] : global_defs[allOpts[k]];
			}
		}
		...
		if (!newOpts[sPRELOAD]) {
			newOpts.cache = newOpts.order = newOpts.xhr = bFALSE; // turn off all flags if preloading is disabled
		}
	...
	*/ 
	(function (addEvent, domLoaded, handler) {
		if (oDOC[sREADYSTATE] === uUNDEFINED && oDOC[addEvent]) {
